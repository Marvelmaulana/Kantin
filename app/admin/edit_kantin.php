...
<head>
    ...
    <title><?= t('admin.edit_kantin_title', 'Edit Kantin - Admin') ?></title>
...
<main class="flex-1 w-full lg:ml-72 p-6 md:p-10 min-h-screen">
    <header class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
            <h2 class="text-3xl font-extrabold"><?= t('admin.edit_kantin_heading', 'Edit Kantin') ?></h2>
            <p class="text-slate-500"><?= t('admin.edit_kantin_desc', 'Perbarui informasi dasar kantin. Kelola penjual di menu Manajemen Penjual.') ?></p>
        </div>
        <a href="manajemen_kantin.php" class="px-4 py-2 rounded-2xl bg-slate-100 text-slate-700"><?= t('action.back', 'Kembali') ?></a>
    </header>
    <?php if ($message !== ''): ?>
    ...
    <form method="POST" class="bg-white rounded-3xl shadow p-6 grid gap-5 max-w-3xl" onsubmit="return confirm('Apakah Anda yakin ingin menyimpan perubahan pada kantin ini?');">
        ...
        <div>
            <label class="block text-sm font-bold mb-2"><?= t('kantin.name_label', 'Nama Kantin') ?></label>
        ...
        <div>
            <label class="block text-sm font-bold mb-2"><?= t('kantin.description_label', 'Deskripsi Singkat') ?></label>
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-end">
            <a href="manajemen_kantin.php" class="px-4 py-3 rounded-2xl border border-slate-200 text-slate-700 text-center hover:bg-slate-50 transition"><?= t('action.cancel', 'Batal') ?></a>
            <button type="submit" class="px-4 py-3 rounded-2xl bg-primary-orange text-white font-bold hover:bg-orange-600 transition"><?= t('action.save', 'Simpan Perubahan') ?></button>
        </div>
    </form>
</main>
</body>
