...
<head>
    ...
    <title><?= t('seller_menu.title') ?></title>
    <style>
        :root {
            --primary-blue: #50c8ff;
            --dark-blue: #3da8db;
...
<body>
    ...
    <div class="sidebar">
        <h2><?= t('buyer.title') ?></h2>
        <div class="user-info">
            <p style="font-size: 11px; opacity: 0.8;"><?= t('seller.welcome') ?>,</p>
            <b><?= htmlspecialchars($_SESSION['username']) ?></b>
        </div>
        <a href="dashboard_penjual.php"><?= t('nav.dashboard') ?></a>
        <a href="menu_penjual.php" class="active"><?= t('nav.manage_menu') ?></a>
        <a href="pesanan_masuk.php"><?= t('nav.incoming_orders') ?></a>
        <a href="logout.php" class="logout"><?= t('nav.logout') ?></a>
    </div>

    <div class="main-content">
        <h1><?= t('seller_menu.manage_menu') ?></h1>

        <a href="tambah_menu.php" class="btn-tambah"><?= t('seller_menu.add_new') ?></a>
